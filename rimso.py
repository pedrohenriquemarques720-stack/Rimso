...
def fazer_cadastro(nome, email, tipo, cidade, bairro, loja_nome=None, categoria=None):
    ...
    st.session_state.user_logado = True
    st.session_state.user_nome = nome if tipo == 'pf' else loja_nome
    st.session_state.user_tipo = tipo
    st.session_state.user_email = email
    st.session_state.modal_cadastro = False
    st.rerun()
    ...
    if st.session_state.aba_ativa == 'inicio':
        ...
        with col1:
            local = st.text_input("", placeholder="Digite cidade, bairro ou rua (ex: Piracicaba, São Paulo)", 
                                 value="Piracicaba - São Paulo", label_visibility="collapsed")
        with col2:
            if st.button("🔍 Buscar", use_container_width=True):
                st.session_state['buscar_local'] = local
        
        # Mapa
        if 'mapa_centro' not in st.session_state:
            st.session_state.mapa_centro = [-23.5505, -46.6333]  # São Paulo
        
        if 'buscar_local' in st.session_state:
            lat, lon, display = buscar_localizacao(st.session_state.buscar_local)
            if lat and lon:
                st.session_state.mapa_centro = [lat, lon]
                st.success(f"📍 Localização encontrada: {display.split(',')[0]}")
            else:
                st.error("Localização não encontrada. Tente novamente.")
            del st.session_state['buscar_local']
        
        # Criar mapa
        m = folium.Map(location=st.session_state.mapa_centro, zoom_start=13, control_scale=True)
        
        # Adicionar marcadores de lojas (exemplo)
        lojas_mapa = [
            {"nome": "StreetWearBR", "lat": st.session_state.mapa_centro[0] - 0.01, 
             "lng": st.session_state.mapa_centro[1] - 0.01, "end": "Rua Principal, 123"},
            {"nome": "UrbanStyle", "lat": st.session_state.mapa_centro[0] + 0.01, 
             "lng": st.session_state.mapa_centro[1] + 0.01, "end": "Av. Central, 1500"},
            {"nome": "Sioostas", "lat": st.session_state.mapa_centro[0], 
             "lng": st.session_state.mapa_centro[1] + 0.02, "end": "Rua Comercial, 350"}
        ]
        
        for loja in lojas_mapa:
            folium.Marker(
                [loja["lat"], loja["lng"]],
                popup=f"<b>{loja['nome']}</b><br>{loja['end']}<br>⭐ 4.5",
...
if st.session_state.modal_cadastro:
    with st.container():
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("👤 Sou Cliente", use_container_width=True):
                st.session_state.form_ativa = 'pf'
        with col2:
            if st.button("🏪 Sou Lojista", use_container_width=True, type="primary"):
        ...
        if st.session_state.form_ativa == 'pf':
            ...
            if st.button("Cadastrar grátis", use_container_width=True, type="primary"):
            ...
            categoria = st.selectbox("Categoria", ["Streetwear", "Sportwear", "Casual", "Moda feminina", "Moda masculina"])
            
            if st.button("💳 Pagar R$20 e cadastrar", use_container_width=True, type="primary"):
            ...
            if st.button("Entrar como lojista", use_container_width=True):
